...
def _view_(page: ft.Page):
    ...
    batas_absen = time(15,30,0)
    datetime_batas_absen = datetime.combine(datetime.now().date(), batas_absen)
    waktu_sekarang = datetime.strptime(datetime.now().time().strftime("%H:%M:%S"),"%H:%M:%S")
    absen_tab = ft.Tabs(
            selected_index=0,
                ...
                ft.Tab(
                    ...
                    content=ft.Column(
                        [
                            ...
                            ft.Row(
                                [
                                    ft.Text(f"{batas_absen.hour:02d}:{batas_absen.minute:02d}:{batas_absen.second:02d}", color="#00BAE9"),  # place holder
                                ],
                                alignment="center",
                            ),
                            ft.Row(
    ...
    no_absent = ft.Row(
            [
                ft.Container(
                    content=ft.Text("Tidak ada absen saat ini!",size=16,color=ft.colors.RED_400,text_align="center",weight=ft.FontWeight.BOLD),
                    width = page.width,
                    height = 30,
                    bgcolor = "white",
                    padding=0,
    ...
    if datetime_batas_absen < waktu_sekarang:
        absent_tab = no_absent
    else:
        absent_tab = absen_tab
